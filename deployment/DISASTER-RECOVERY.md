...
- **Compression**: gzip compression (~90% size reduction)
...
```
/backups/
├── latest/       # Most recent backup (always latest.sql.gz)
├── daily/        # Last 7 days
...
└── daily/catalyst_backup_<timestamp>.sql.gz
```

### Backup File Format

- **Format**: PostgreSQL custom format (compressed)
- **Filename**: `catalyst_backup_YYYYMMDD_HHMMSS.sql.gz`
- **Example**: `catalyst_backup_20260119_020000.sql.gz`
...
4. **Download latest backup from S3** (if cloud backups enabled):
   ```bash
   aws s3 cp s3://<bucket>/catalyst-backups/latest/latest.sql.gz /tmp/backup.sql.gz
   ```
...
7. **Restore database:**
   ```bash
   docker run --rm \
     --network deployment_catalyst-network \
     -v /tmp/backup.sql.gz:/backup.sql.gz \
     ...
     -e FORCE=true \
     postgres:16-alpine \
     sh -c "pg_restore -h catalyst-postgres -U catalyst -d catalyst --no-owner --no-acl /backup.sql.gz"
   ```
...
3. **Restore from specific date:**
   ```bash
   docker exec -it catalyst-backup \
     bash -c "FORCE=true /usr/local/bin/restore-postgres.sh /backups/daily/daily_20260119.sql.gz"
   ```
...
2. **Copy backup file to PostgreSQL container:**
   ```bash
   docker cp /path/to/backup.sql.gz catalyst-postgres:/tmp/backup.sql.gz
   ```
...
4. **Restore backup:**
   ...
   docker exec -it catalyst-postgres \
     pg_restore -U catalyst -d catalyst --no-owner --no-acl /tmp/backup.sql.gz
   ```
...
1. **Backup current state** (if possible):
   ...
   docker cp catalyst-backup:/backups/latest/latest.sql.gz /safe/location/
   ```
...
8. **Restore database:**
   ```bash
   docker exec -it catalyst-postgres \
     pg_restore -U catalyst -d catalyst --clean --if-exists /backups/latest/latest.sql.gz
   ```
...
- TCF v2 consent string format validation
- Database dump format validation
- File size > 0 check

View verification logs:
```bash
docker logs catalyst-backup | grep "integrity verified"
```
...
| Restore latest | `docker exec catalyst-backup bash -c "FORCE=true /usr/local/bin/restore-postgres.sh /backups/latest/latest.sql.gz"` |
| Check backup logs | `docker logs catalyst-backup` |
| List all backups | `docker exec catalyst-backup find /backups -name "*.sql.gz" -ls` |
| Download from S3 | `aws s3 cp s3://$BACKUP_S3_BUCKET/catalyst-backups/latest/latest.sql.gz .` |
...
docker exec catalyst-backup find /backups/daily -name "daily_$(date +%Y%m%d).sql.gz"
```
