...
func TestMakeBids_Success(t *testing.T) {
	...
	responseBody := `{
		...
		"cur": "USD",
		"seatbid": [{
			"bid": [{"id": "bid-1", "impid": "imp-1", "price": 1.75}]
		}]
	}`
	...
	if bidderResponse.Bids[0].Bid.Price != 1.75 {
		t.Errorf("Expected price 1.75, got %f", bidderResponse.Bids[0].Bid.Price)
	}
}
...
func TestInfo(t *testing.T) {
	...
	if info.GVLVendorID != 76 {
		t.Errorf("Expected GVL vendor ID 76, got %d", info.GVLVendorID)
	}
}
