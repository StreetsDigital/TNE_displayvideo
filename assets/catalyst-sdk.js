...
(function(window) {
  ...
  catalyst._setSlotTargeting = function(bid) {
    ...
    try {
      ...
      if (targetSlot) {
        // Set standard header bidding keys
        if (bid.cpm) {
          targetSlot.setTargeting('hb_pb_catalyst', bid.cpm.toFixed(2));
          targetSlot.setTargeting('hb_pb', bid.cpm.toFixed(2));
        }
        ...
        if (bid.width && bid.height) {
          ...
          targetSlot.setTargeting('hb_size', bid.width + 'x' + bid.height);
        }

        // Set bidder wrapper
        targetSlot.setTargeting('hb_bidder', 'catalyst');
        ...
        // Set page-level targeting if no slot found
        if (bid.cpm) {
          pubads.setTargeting('hb_pb_catalyst', bid.cpm.toFixed(2));
        }
        pubads.setTargeting('hb_bidder', 'catalyst');
